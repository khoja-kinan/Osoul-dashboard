...
import { CategorySubscriptionListUrl } from "../../../constants/urls";

// ----------------------------------------------------------------------

export default function CategorySubscriptionMoreMenu({
  item,
  token,
  UsersList,
  CategoriesList,
  ...
  */
  ...
  const [Area, setArea] = useState(item.category_id);
  ...
  const handleConfirmEditCity = () => {
    ...
    axios
      .put(`${CategorySubscriptionListUrl}/${item.id}`, data, { headers })
      .then((response) => {
  ...
  const handleConfirmDelete = () => {
    axios
      .delete(`${CategorySubscriptionListUrl}/${item.id}`, {
        headers: {
          Accept: "application/json",
          Authorization: "Bearer " + token,
        },
