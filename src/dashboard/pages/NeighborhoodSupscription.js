...
import {
  NeighboorhoodSubscriptionMoreMenu,
  UserListHead,
  UserListToolbar,
...
import {
  ...
  neighborhoodSubscriptionListUrl,
  userListUrl,
} from "../constants/urls";
...
export default function NeighborhoodSupscription() {
  ...
  const [Neighboorhood, setNeighboorhood] = useState("");
  ...
  async function fecthData() {
    await axios
      .get(neighborhoodSubscriptionListUrl, {
        headers: {
      ...
      .then((response) => {
        if (response.status === 200) {
          setNeighborhoodSubscriptions(response.data.data);
        }
      })
      .catch((error) => {
  ...
  if (
    NeighborhoodSubscriptions === undefined ||
    UsersList === undefined ||
    neighboorhoodList === undefined
  ) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = NeighborhoodSubscriptions.map((n) => n.name);
  ...
  const emptyRows =
    page > 0
      ? Math.max(0, (1 + page) * rowsPerPage - NeighborhoodSubscriptions.length)
      : 0;
  ...
  const handleChangeNeighboorhood = (e) => {
    setNeighboorhood(e.target.value);
  };

  const handleAddNew = () => {
    setBtnLoading(true);
    const data = { user_id: User, neighborhood_id: Neighboorhood };
    const headers = {
      Authorization: "Bearer " + token,
      Accept: "application/json",
    };
    axios
      .post(neighborhoodSubscriptionListUrl, data, { headers })
      .then((response) => {
        setOpenNewDialog(false);
        setBtnLoading(false);
  ...
  return (
    <Page title="لوحة التحكم | اشتراكات الأحياء">
      <Container>
        <Stack
          direction="row"
          alignItems="center"
        ...
        >
          ...
          <DialogContent sx={{ width: "20rem" }}>
            ...
            >
              <FormControl sx={{ m: 1, minWidth: "100%" }}>
                <InputLabel id="demo-dialog-select-label">الحي</InputLabel>
                <Select
                  labelId="demo-dialog-select-label"
                  id="demo-dialog-select"
                  value={Neighboorhood}
                  onChange={handleChangeNeighboorhood}
                  input={<OutlinedInput label="الحي" />}
                >
                  {neighboorhoodList.map((neighboorhood) => (
                    <MenuItem value={neighboorhood.id} key={neighboorhood.id}>
                      {" "}
                      {neighboorhood.name}{" "}
                    </MenuItem>
                  ))}
        ...
        <Card>
            ...
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                <UserListHead
                  ...
                  rowCount={NeighborhoodSubscriptions.length}
                  numSelected={selected.length}
                        ...
                        >
                          ...
                          <TableCell align="left">
                            <NeighboorhoodSubscriptionMoreMenu
                              ...
                              token={token}
                              UsersList={UsersList}
                              neighboorhoodList={neighboorhoodList}
                            />
                          </TableCell>
                          {/* )} */}
                        </TableRow>
          ...
          <TablePagination
            ...
            component="div"
            count={NeighborhoodSubscriptions.length}
